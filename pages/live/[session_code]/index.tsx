...
const DirectLive = (props: IProps) => {
  ...
  useEffect(() => {
    if (eventDetails) {
      ...
      const endtime = Number(eventDetails.endTime)
      if (countDownDate) {
    ...
    if (eventDetails.session_status) {
      switch (eventDetails.session_status) {
        case "LIVE":
          ...
          break;
        case "UPCOMING":
          setEventHeader("Details of your upcoming event:")
              ...
              <Box className="">
                <Grid container >
                  <div className="main-description">
                    {/* <p className="mb-0">{`${eventDetails.location} is set to open on:`}</p>
                    <p className="mt-0">{sessionDetails && sessionDetails.scheduled_start_at && moment.unix(sessionDetails.scheduled_start_at / 1000).format("MMMM D, YYYY")} at {sessionDetails && sessionDetails.scheduled_start_at && moment.unix(sessionDetails.scheduled_start_at / 1000).format("h:mmA z")} </p> */}
                  </div>
